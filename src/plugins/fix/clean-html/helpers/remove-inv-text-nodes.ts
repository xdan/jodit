...
export function removeInvTextNodes(jodit: IJodit): void {
	const currentNode = jodit.s.current();

	if (!currentNode) {
		return;
	}

	const currentParagraph = Dom.up(currentNode, Dom.isBlock, jodit.editor);

	if (!currentParagraph) {
		return;
	}

	Dom.each(currentParagraph, node => {
