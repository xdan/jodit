 ...
 * @return {Element}
 */
export const insertParagraph = (editor, fake) => {
    let p = editor.doc.createElement(editor.options.enter),
 ...
 * @return {Element}
 */
export const wrap = (current, range, editor) => {
...
Jodit.plugines.enter = function (editor) {
    ...
    editor.events.on('keydown', (event) => {
        if (event.which === consts.KEY_ENTER || event.keyCode === consts.KEY_ENTER) {
            ...
            if (currentBox) {
                ...
                } else if (editor.node.next(current, (elm) => (elm), currentBox) || (current.nodeType === Node.TEXT_NODE && range.startOffset !== current.nodeValue.length)) {
