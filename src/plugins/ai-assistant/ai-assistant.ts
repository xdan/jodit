...
export class aiAssistant extends Plugin {
    ...
    override afterInit(jodit: IJodit): void {
        jodit.e.on('invokeAiAssistant.ai-assistant', (prompt: string, aiAssistantCallback: (prompt: string, htmlFragment: string) => Promise<string>) => {
            jodit.s.focus();
            const selectedText = jodit.s.html;
            aiAssistantCallback(prompt, selectedText).then((htmlFragment: string) => {
        ...
        jodit.e.on('generateAiAssistantForm.ai-assistant', (prompt: string) => {
            ...
            const body = container.querySelector('.jodit-ai-assistant__container__body');
            const results = container.querySelector('.jodit-ai-assistant__container__results');
            const containerButtons = container.querySelector('.jodit-ai-assistant__container__buttons');

            const insertAfterButton = Button(jodit, '', 'Insert After', 'initial').onAction(() => {
                if (results!.firstChild !== null) {
                    const currentSelection = jodit.s.current();
                    if (currentSelection !== null && currentSelection.parentElement !== null) {
                        jodit.s.setCursorIn(currentSelection.parentElement, false);
                        jodit.s.insertHTML(results!.innerHTML);
                    }
                }
                dialog.close();
            });
            let tryAgainButton: IUIButton;
            ...
            const promptInput = new UITextArea(jodit, {
                ...
                className: 'jodit-ai-assistant__container__body__prompt-row__input'
            });
            const insertButton = Button(jodit, '', 'Insert', 'primary').onAction(() => {
                if (results!.firstChild !== null) {
                    jodit.s.focus();
                    jodit.s.insertHTML(results!.innerHTML);
                }
            ...
            formAiAssistant.onSubmit((data: Record<string, string>) => {
                jodit.e.fire('invokeAiAssistant', data.prompt, jodit.o.aiAssistantCallback);
            ...
            if (prompt !== null && prompt !== undefined && prompt !== '') {
                ...
                if (promptToUse !== null && promptToUse !== undefined && promptToUse !== '') {
