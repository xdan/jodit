...
__webpack_require__.d(helpers_namespaceObject, "defaultLanguage", function() { return defaultLanguage; });
...
__webpack_require__.d(plugins_namespaceObject, "size", function() { return size; });
...
var bundle = __webpack_require__(78);
 ...
 * Copyright (c) 2013-2019 Valeriy Chupurnov. All rights reserved. https://xdsoft.net
 */
const setTimeout_setTimeout = (callback, timeout, a1, a2, a3) => {
    if (!timeout) {
...
const appendScript = (url, callback, className = '', doc) => {
...
const css_css = (element, key, value, onlyStyleMode = false) => {
    ...
    if (isPlainObject(key) || value !== undefined) {
        const setValue = (elm, _key, _value) => {
            ...
            if (_value !== undefined &&
                css_css(elm, _key, void 0, true) !== normilizeCSSValue(_key, _value)) {
    ...
    let result = currentValue !== undefined && currentValue !== ''
        ? currentValue
        : win && !onlyStyleMode
 ...
 * Copyright (c) 2013-2019 Valeriy Chupurnov. All rights reserved. https://xdsoft.net
 */
const defaultLanguage = (language) => language === 'auto' || language === undefined
...
class ToolbarIcon {
    ...
    static getIcon(name, defaultValue = '<span></span>') {
        const icon = ToolbarIcon.icons[name] ||
...
const OptionsDefault = function (options) {
    const def = Config_Config.defaultOptions, self = this;
...
class observer_Observer extends Component_Component {
    constructor(editor) {
        ...
        this.onChangeStack = () => {
            ...
            if (!Snapshot_Snapshot.equal(this.__newValue, this.__startValue)) {
        ...
        editor.events.on('afterInit.observer', () => {
            ...
            editor.events
                .on('changeSelection.observer selectionstart.observer selectionchange.observer mousedown.observer mouseup.observer keydown.observer keyup.observer', () => {
                if (this.__startValue.html ===
...
class Create_Create {
    constructor(ownerDocument, editorDocument) {
        this.doc = ownerDocument;
        if (editorDocument !== null) {
            this.inside = editorDocument
                ? new Create_Create(editorDocument)
                : new Create_Create(ownerDocument, null);
        }
    }
    setDocument(doc) {
        this.doc = doc;
...
class panel_Panel extends Component_Component {
    constructor(jodit) {
        ...
        this.create = new Create_Create(this.ownerDocument, isJoditObject(jodit) ? jodit.editorDocument : undefined);
...
class view_View extends panel_Panel {
    constructor(jodit, options) {
        ...
        this.events =
            jodit && jodit.events
                ? jodit.events
                : new eventsNative_EventsNative(this.ownerDocument);
        this.buffer = jodit && jodit.buffer ? jodit.buffer : storage_Storage.makeStorage();
    ...
    i18n(text, ...params) {
        return this.jodit && this.jodit !== this
            ...
            : Jodit_Jodit.prototype.i18n(text, ...params);
    }
    toggleFullSize(isFullSize) {
...
class popup_Popup extends Component_Component {
    ...
    open(content, rightAlign, noStandartActions = false) {
...
// CONCATENATED MODULE: ./src/modules/toolbar/tooltip.ts
/*!
 * Jodit Editor (https://xdsoft.net/jodit/)
 * Licensed under GNU General Public License version 2 or later or a commercial license or MIT;
 * For GPL see LICENSE-GPL.txt in the project root for license information.
...
class tooltip_ToolbarTooltip extends element_ToolbarElement {
    constructor(button) {
    ...
    constructor(parentToolbarOrView, control, target) {
        ...
        this.onMouseDown = (originalEvent) => {
            ...
            if (control.list) {
                ...
                control.exec(this.jodit, getTarget(), control, originalEvent, this.container);
                this.jodit.events.fire('synchro');
        ...
        if (this.jodit.options.showTooltip && control.tooltip) {
            if (!this.jodit.options.useNativeTooltip) {
                this.tooltip = new tooltip_ToolbarTooltip(this);
            }
            else {
...
class collection_ToolbarCollection extends Component_Component {
    constructor(jodit) {
        ...
        this.checkActiveButtons = debounce(this.immedateCheckActiveButtons, this.jodit.defaultTimeout);
    ...
    build(buttons, container, target) {
...
class viewWithToolbar_ViewWithToolbar extends view_View {
    constructor() {
        super(...arguments);
        this.toolbar = joditToolbarCollection_JoditToolbarCollection.makeCollection(this);
    }
    destruct() {
        this.toolbar.destruct();
        delete this.toolbar;
...
class Jodit_Jodit extends viewWithToolbar_ViewWithToolbar {
    constructor(element, options) {
        ...
        this.i18n = (key, ...params) => {
            const debug = this.options !== undefined && this.options.debugLanguage;
            let store;
            ...
            const parse = (value) => params.length ? sprintf(value, params) : value, default_language = Config_Config.defaultOptions.language === 'auto'
                ? defaultLanguage(Config_Config.defaultOptions.language)
                : Config_Config.defaultOptions.language, language = defaultLanguage(this.options ? this.options.language : default_language);
            if (this.options !== undefined && Jodit_Jodit.lang[language] !== undefined) {
                store = Jodit_Jodit.lang[language];
            }
            else {
                if (Jodit_Jodit.lang[default_language] !== undefined) {
                    store = Jodit_Jodit.lang[default_language];
                }
                else {
            ...
            if (typeof Jodit_Jodit.lang.en[key] === 'string' && Jodit_Jodit.lang.en[key]) {
        ...
        if (typeof element === 'string') {
            try {
                this.element = this.ownerDocument.querySelector(element);
            }
            catch (_a) {
        ...
        this.observer = new observer_Observer(this);
        let buffer = null;
        ...
        this.statusbar = new StatusBar_StatusBar(this, this.container);
    ...
    __initPlugines() {
        ...
        Object.keys(Jodit_Jodit.plugins).forEach((key) => {
            if (disable.indexOf(key.toLowerCase()) === -1) {
                this.__plugins[key] = new Jodit_Jodit.plugins[key](this);
            }
        });
    }
    async __initEditor(buffer) {
    ...
    async __createEditor() {
        const defaultEditorAreae = this.editor;
        const stayDefault = await this.events.fire('createEditor', this);
        if (this.isDestructed) {
            return;
        }
        if (stayDefault === false) {
            Dom_Dom.safeRemove(defaultEditorAreae);
        }
        ...
        if (this.options.direction) {
            const direction = this.options.direction.toLowerCase() === 'rtl' ? 'rtl' : 'ltr';
    ...
    destruct() {
        ...
        if (this.element !== this.container) {
            if (this.element.hasAttribute(this.__defaultStyleDisplayKey)) {
                const attr = this.element.getAttribute(this.__defaultStyleDisplayKey);
                if (attr) {
        ...
        Dom_Dom.safeRemove(this.workplace);
        Dom_Dom.safeRemove(this.editor);
        Dom_Dom.safeRemove(this.progress_bar);
        Dom_Dom.safeRemove(this.iframe);
...
// CONCATENATED MODULE: ./src/langs/ar.ts
...
// CONCATENATED MODULE: ./src/langs/cs_cz.ts
/* harmony default export */ var cs_cz = ({
    ...
    'GNU General Public License, version 2 or later': 'Licence GNU (GPL), verze 2 nebo vyšší',
...
// CONCATENATED MODULE: ./src/langs/de.ts
/* harmony default export */ var de = ({
    ...
    'The pasted content is coming from a Microsoft Word/Excel document. Do you want to keep the format or clean it up?': 'Der Inhalt, den Sie einfügen, stammt aus einem Microsoft Word / Excel-Dokument. Möchten Sie das Format ' +
    ...
    Remove: 'Entfernen',
    Select: 'Markieren',
    'You can only edit your own images. Download this image on the host?': 'Sie können nur Ihre eigenen Bilder bearbeiten.' +
...
// CONCATENATED MODULE: ./src/langs/es.ts
/* harmony default export */ var es = ({
    ...
    'GNU General Public License, version 2 or later': 'GNU General Public License, Versión 2 o posterior',
...
// CONCATENATED MODULE: ./src/langs/fr.ts
/* harmony default export */ var fr = ({
    ...
    'The pasted content is coming from a Microsoft Word/Excel document. Do you want to keep the format or clean it up?': "Le contenu que vous insérez provient d'un document Microsoft Word / Excel. " +
...
// CONCATENATED MODULE: ./src/langs/he.ts
/* harmony default export */ var he = ({
...
// CONCATENATED MODULE: ./src/langs/hu.ts
...
// CONCATENATED MODULE: ./src/langs/id.ts
/* harmony default export */ var langs_id = ({
    'Type something': 'Ketik sesuatu',
    'About Jodit': 'Tentang Jodit',
    'Jodit Editor': 'Editor Jodit',
    'Free Non-commercial Version': 'Versi Bebas Non-komersil',
    'GNU General Public License, version 2 or later': 'Lisensi Publik Umum (GPL), versi 2 atau terbaru',
...
// CONCATENATED MODULE: ./src/langs/it.ts
...
// CONCATENATED MODULE: ./src/langs/nl.ts
...
// CONCATENATED MODULE: ./src/langs/pt_br.ts
/* harmony default export */ var pt_br = ({
    ...
    'GNU General Public License, version 2 or later': 'Licença Pública GNU General, Versão 2 ou posterior',
    ...
    Cancel: 'Cancelar',
    Ok: 'Ok',
    'Your code is similar to HTML. Keep as HTML?': 'Seu código é simular ao HTML. Manter como HTML?',
    ...
    Remove: 'Remover',
    Select: 'Selecionar',
    'You can only edit your own images. Download this image on the host?': 'Você só pode editar suas próprias imagens. ' +
...
// CONCATENATED MODULE: ./src/langs/ru.ts
/* harmony default export */ var ru = ({
    ...
    'The pasted content is coming from a Microsoft Word/Excel document. Do you want to keep the format or clean it up?': 'Контент который вы вставляете поступает из документа Microsoft Word / Excel. Вы хотите сохранить ' +
...
// CONCATENATED MODULE: ./src/langs/tr.ts
/* harmony default export */ var langs_tr = ({
    'Type something': 'Bir şey yazın.',
    Advanced: 'Gelişmiş',
    'About Jodit': 'Jodit Hakkında',
    'Jodit Editor': 'Jodit Editor',
    'Free Non-commercial Version': 'Ücretsiz, ticari olmayan versiyon',
    'GNU General Public License, version 2 or later': 'GNU General Public License, Versiyon 2 ve sonrası için',
    ...
    Remove: 'Sil',
    Select: 'Seç',
    'You can only edit your own images. Download this image on the host?': 'Sadece kendi resimlerinizi düzenleyebilirsiniz.' +
...
// CONCATENATED MODULE: ./src/langs/zh_cn.ts
...
// CONCATENATED MODULE: ./src/langs/zh_tw.ts
/* harmony default export */ var zh_tw = ({
    ...
    'Split vertical': '垂直拆分',
    'Split horizontal': '水平拆分',
    'You can only edit your own images. Download this image on the host?': '妳只能編輯妳自己的圖片。' + 'Download this image on the host?',
...
// CONCATENATED MODULE: ./src/langs/en.ts
            ...
            this.xhr.onreadystatechange = () => {
                if (this.xhr.readyState === XMLHttpRequest.DONE) {
                    ...
                    if (this.success_response_codes.indexOf(this.xhr.status) > -1) {
                        ...
                        reject.call(this.xhr, new Error(this.xhr.statusText ||
...
class dialog_Dialog extends view_View {
    constructor(jodit, options = Config_Config.prototype.dialog) {
        ...
        self.toolbar = joditToolbarCollection_JoditToolbarCollection.makeCollection(self);
        ...
        Jodit_Jodit.plugins.fullsize(self);
    }
...
Config_Config.prototype.controls.filebrowser = {
    ...
    sort: {
        isInput: true,
        getContent: (fb) => {
            const select = fb.create.fromHTML('<select class="jodit_input">' +
...
class fileBrowser_FileBrowser extends viewWithToolbar_ViewWithToolbar {
    constructor(editor, options) {
        ...
        this.status = (message, success) => {
            ...
            this.statusTimer = setTimeout_setTimeout(() => {
    ...
    async loadTree() {
        ...
        if (this.options.showFoldersPanel) {
            const tree = this.dataProvider
                ...
                .catch((e) => {
                this.errorHandler(new Error(this.jodit.i18n('Error on load folders')));
                error(e);
            });
    ...
    initNativeEventsListeners() {
        ...
        self.events
            ...
            const a = this.parentNode, path = a.getAttribute('data-path') || '';
            ...
            .on(self.tree, 'click', function (e) {
            const a = this.parentNode, name = a.getAttribute('data-name') || '', path = a.getAttribute('data-source-path') || '';
            Promt(self.i18n('Enter new name'), self.i18n('Rename'), (newName) => {
                if (!isValidName(newName)) {
                    self.status(self.i18n('Enter new name'));
                    return false;
    ...
    constructor(editor) {
        ...
        this.showCrop = () => {
            ...
            css_css(this.cropHandler, {
                ...
                width: this.new_w,
                height: this.new_h,
                left: (this.cropImage.offsetWidth || this.image.offsetWidth) / 2 -
                    this.new_w / 2,
                top: (this.cropImage.offsetHeight || this.image.offsetHeight) / 2 -
                    this.new_h / 2
            });
        ...
        this.open = (url, save) => {
            return new Promise(resolve => {
                ...
                const onload = () => {
                    ...
                    this.cropImage = this.image.cloneNode();
        ...
        this.buttons = [
            this.jodit.create.fromHTML('<button data-action="reset" type="button" class="jodit_btn">' +
                gi('update') +
                '&nbsp;' +
                i('Reset') +
                '</button>'),
            this.jodit.create.fromHTML('<button data-action="save" type="button" class="jodit_btn jodit_btn_success">' +
                gi('save') +
                '&nbsp;' +
                i('Save') +
                '</button>'),
            this.jodit.create.fromHTML('<button data-action="saveas" type="button" class="jodit_btn jodit_btn_success">' +
                gi('save') +
                '&nbsp;' +
                i('Save as ...') +
                '</button>')
...
function addNewLine(editor) {
    ...
    const hide = () => {
        ...
        clearTimeout(timeout);
        timeout = setTimeout_setTimeout(hideForce, 500);
    };
    editor.events
        .on('beforeDestruct', () => {
    ...
    editor.events
        ...
        if (editor.options.autofocus) {
            if (editor.defaultTimeout) {
                timeout = setTimeout_setTimeout(editor.selection.focus, 300);
            }
            else {
    ...
    editor.events
        ...
        if (command === 'delete') {
            ...
            if (current &&
                ...
                Dom_Dom.safeRemove(current.firstChild);
            }
...
function paste(editor) {
    const clearOrKeep = (msg, title, callback, clearButton = 'Clean', clear2Button = 'Insert only Text') => {
        if (editor.events &&
        ...
        const keep = dialog.create.fromHTML('<a href="javascript:void(0)" style="float:left;" class="jodit_button">' +
            '<span>' +
            editor.i18n('Keep') +
            '</span>' +
            '</a>');
        const clear = dialog.create.fromHTML('<a href="javascript:void(0)" style="float:left;" class="jodit_button">' +
            '<span>' +
            editor.i18n(clearButton) +
            '</span>' +
            '</a>');
        const clear2 = dialog.create.fromHTML('<a href="javascript:void(0)" style="float:left;" class="jodit_button">' +
            '<span>' +
    ...
    if (editor.options.askBeforePasteHTML) {
        editor.events.on('beforePaste', (event) => {
            ...
            if (event && dt && dt.getData(TEXT_PLAIN)) {
                const html = dt.getData(TEXT_PLAIN);
            ...
            if (event && dt && dt.getData && dt.getData(TEXT_HTML)) {
                ...
                if (event.type !== 'drop') {
                    ...
                    const waitData = () => {
                        ...
                        if (tick < 5) {
                            setTimeout_setTimeout(waitData, 20);
                        }
                        else {
                            removeFakeFocus();
                        }
    ...
    if (editor.options.nl2brInPlainText) {
        editor.events.on('processPaste', (event, text, type) => {
...
class cut_clipboard extends Plugin_Plugin {
    afterInit(editor) {
        editor.events
...
class paste_storage_pasteStorage extends Plugin_Plugin {
    ...
    afterInit() {
        this.jodit.events.on('afterCopy', (html) => {
...
class drag_and_drop_element_DragAndDropElement extends Plugin_Plugin {
    constructor() {
        ...
        this.onDragStart = (event) => {
            ...
            this.onDragEnd();
            this.timeout = setTimeout_setTimeout((lastNode) => {
                if (!lastNode) {
                    return;
                }
...
function imageProperties(editor) {
    const open = function (e) {
        ...
        const dom = editor.create.fromHTML.bind(editor.create), image = this, dialog = new dialog_Dialog(editor), cancel = dom('<a href="javascript:void(0)" style="float:right;" class="jodit_button">' +
            ToolbarIcon.getIcon('cancel') +
            '<span>' +
            editor.i18n('Cancel') +
        ...
        let timer, lockSize = true, lockMargin = true;
        ...
        editor.events.on([$w, $h], 'change keydown mousedown paste', (event) => {
            ...
            if (editor.defaultTimeout) {
                clearTimeout(timer);
                timer = setTimeout_setTimeout(changeSizes.bind(this, event), editor.defaultTimeout);
            }
            else {
                changeSizes(event);
            }
...
function limit(jodit) {
    if (jodit && (jodit.options.limitWords || jodit.options.limitChars)) {
        ...
        jodit.events
            ...
            .on('keydown keyup beforeEnter beforePaste', (event) => {
            if (callback(event) !== undefined) {
                return false;
            }
        })
            .on('change', debounce((newValue, oldValue) => {
...
Config_Config.prototype.controls.link = {
    ...
    popup: (editor, current, self, close) => {
        const sel = editor.selection.sel, form = editor.create.fromHTML('<form class="jodit_form">' +
        ...
        const lnk = form.querySelector('.jodit_link_insert_button'), unlink = form.querySelector('.jodit_unlink_button');
        if (current) {
            val_val(form, 'input[name=url]', current.getAttribute('href') || '');
            val_val(form, 'input[name=text]', current.textContent || '');
            ...
            val_val(form, 'input[name=text]', sel ? sel.toString() : '');
            if (lnk) {
        ...
        form.addEventListener('submit', (event) => {
            ...
            a.setAttribute('href', val_val(form, 'input[name=url]'));
            a.textContent = val_val(form, 'input[name=text]');
            if (editor.options.link.openInNewTabCheckbox) {
...
function link_link(jodit) {
    ...
    if (jodit.options.link.removeLinkAfterFormat) {
        jodit.events.on('afterCommand', (command) => {
...
function resizer(editor) {
    ...
    let handle, currentElement, resizeElementClicked = false, isResizing = false, start_x, start_y, width, height, ratio, new_h, new_w, diff_x, diff_y, resizerIsVisible = false, timeoutSizeViewer = 0;
    const resizerElm = editor.create.fromHTML('<div data-editor_id="' +
        editor.id +
        ...
        timeoutSizeViewer = setTimeout_setTimeout(hideSizeViewer, editor.options.resizer.hideSizeTimeout);
    }, updateSize = () => {
    ...
    editor.events
        ...
        editor.events
            ...
            .on(editor.ownerWindow, 'mousemove touchmove', (e) => {
            if (isResizing) {
            ...
            .on(editor.ownerWindow, 'mouseup keydown touchend', (e) => {
...
// CONCATENATED MODULE: ./src/plugins/source.ts
/*!
 * Jodit Editor (https://xdsoft.net/jodit/)
 * Licensed under GNU General Public License version 2 or later or a commercial license or MIT;
 * For GPL see LICENSE-GPL.txt in the project root for license information.
...
class source_source extends Plugin_Plugin {
    constructor() {
        ...
        this.selInfo = [];
        this.lastTuple = null;
        this.loadNext = (i, urls, eventOnFinalize = 'aceReady', className = this.className) => {
            ...
            if (urls[i] !== undefined) {
                ...
                this.lastTuple = appendScript(urls[i], () => {
                    if (!this.isDestructed) {
        ...
        this.insertHTML = (html) => {
            if (this.mirror.selectionStart || this.mirror.selectionStart === 0) {
                const startPos = this.mirror.selectionStart, endPos = this.mirror.selectionEnd;
                this.mirror.value =
        ...
        this.restoreSelection = () => {
            ...
            try {
                ...
                if (this.jodit.ownerWindow.html_beautify &&
        ...
        this.__proxyOnFocus = (e) => {
            this.jodit.events.fire('focus', e);
        };
        this.__proxyOnMouseDown = (e) => {
        ...
        this.onReadonlyReact = () => {
            const isReadOnly = this.jodit.options.readonly;
            if (isReadOnly) {
    ...
    replaceMirrorToACE() {
        ...
        let aceEditor, undoManager;
        const updateButtons = () => {
            if (undoManager &&
                editor.getRealMode() === MODE_SOURCE) {
            ...
            const rows = aceEditor.session.getLength();
            ...
            const lastColumnIndices = getLastColumnIndices();
            ...
            if (aceEditor === undefined &&
                this.jodit.ownerWindow.ace !== undefined) {
                this.jodit.events.off(this.jodit.ownerWindow, 'aceReady', tryInitAceEditor);
                ...
                this.insertHTML = (html) => {
                    const start = aceEditor.selection.getCursor(), end = aceEditor.session.insert(start, html);
    ...
    afterInit(editor) {
        ...
        editor.events
            ...
            .on(this.mirror, 'mousedown focus', (e) => {
            editor.events.fire(e.type, e);
        });
        editor.events
            .on('setMinHeight.source', (minHeightD) => {
            this.mirror && css_css(this.mirror, 'minHeight', minHeightD);
        })
    ...
    beforeDestruct(jodit) {
        ...
        Dom_Dom.safeRemove(this.mirror);
...
class sticky_sticky extends Plugin_Plugin {
    ...
    beforeDestruct(jodit) {
        Dom_Dom.safeRemove(this.dummyBox);
    }
}
...
class table_TableProcessor extends Plugin_Plugin {
    ...
    hideResizer() {
        clearTimeout(this.hideTimeout);
        this.hideTimeout = setTimeout_setTimeout(() => {
...
class xpath_xpath extends Plugin_Plugin {
    constructor() {
        ...
        this.tpl = (bindElement, path, name, title) => {
            const li = this.jodit.create.fromHTML('<li>' +
                '<a ' +
                'role="button" ' +
                'data-path="' +
                path +
...
Object.keys(icons_namespaceObject)
    .filter(esFilter)
    .forEach((key) => {
    ToolbarIcon.icons[key.replace('_', '-')] = icons_namespaceObject[key];
});
...
Object.keys(plugins_namespaceObject)
    .filter(esFilter)
    .forEach((key) => {
    Jodit_Jodit.plugins[key] = plugins_namespaceObject[key];
});
Object.keys(langs)
    .filter(esFilter)
    .forEach((key) => {
