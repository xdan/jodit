describe('Selection Module Tests', function() {
	describe('Current method', function() {
		...
		describe('Cursor in the left of some SPAN', function() {
			it('Should return text before this span', function() {
				...
				expect(editor.selection.current()).to.be.equal(
			...
			Jodit.make(appendTestArea(), {
				...
				events: {
					...
					aceInited: function(jodit) {
						...
						expect(
							jodit.__plugins.source.aceEditor.getSelectionRange()
								.start.column
						).to.equal(258);
						expect(
							jodit.__plugins.source.aceEditor.getSelectionRange()
						...
						jodit.__plugins.source.aceEditor.session.insert(
							jodit.__plugins.source.aceEditor.getCursorPosition(),
							' world'
						);

						expect(
							jodit.__plugins.source.aceEditor.getValue()
						).to.equal(
	...
	describe('Method eachSelection', function() {
		it('Should call callback for each node in selection', function() {
			...
			).to.be.equal(nodesNames.toString().toLowerCase());
		});
		it('Should call callback for each node in selection range', function() {
			...
			expect(['p', 'strong', 'p'].toString().toLowerCase()).to.be.equal(
		...
		it('Should not call callback for editor', function() {
			...
			expect(['#text'].toString().toLowerCase()).to.be.equal(
				nodesNames.toString().toLowerCase()
			);
		});
